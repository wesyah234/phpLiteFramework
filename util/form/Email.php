...
class PLF_Email extends PLF_Element
{
  var $maxlength;
  var $size;
  ...
  function PLF_Email($name, $label, $size, $maxlength, $required) {
    self:
    $this->__construct($name, $label, $size, $maxlength, $required);
  }

  // MyForm class will set tabindex before calling this method
  function render($tabIndex) {
    $toReturn = ' <input  '.$this->getDisabledAttribute().' type="text" tabindex="'.$tabIndex.'" name="'.$this->getName().'" id="'.$this->getName().'" size="'.$this->size.'" maxlength="'.$this->maxlength.'"';
  ...
  function validate() {
    ...
    if (parent::validate($this->value)) {
      ...
      elseif (strlen($this->value ?? '') > 0 && !filter_var($this->value, FILTER_VALIDATE_EMAIL)) {
        $this->requiredText = 'please enter a valid email address';
      }
      else {
        return true;
      }
