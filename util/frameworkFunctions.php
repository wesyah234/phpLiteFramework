...
function sendMail_private($replyToBounceAddress, $fromAddress, $fromName, $toNamesAndAddresses, $subject, $text, $html = NULL, $attachmentFilename = null) {
  ...
  // version 5 is the last version that supports php 5.
  // version 6 requires php 7
  require_once FRAMEWORKDIR.'/thirdParty/mail/swiftmailer-5.4.12/lib/swift_required.php';
  try {
    //Create the Transport
    $transport = Swift_SmtpTransport::newInstance(MAIL_SMTPSERVER, MAIL_SMTPPORT);
    if (MAIL_SMTPSECURE) {
    ...
    $message->setTo($toNamesAndAddresses);
    $mailer->send($message);
